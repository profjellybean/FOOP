...
const addRect = () => {
  ...
  rectStore.addRect(rect)
  peerService.addRect(rect)
}

const animationMaps = new Map<number, number>()

const toggleAnimateRect = (rect: Rect) => {
  if (rect.animate) {
    window.cancelAnimationFrame(animationMaps.get(rect.id)!);
    rect.animate = false
  } else {
    rect.animate = true
    animateRect(rect)
...
const animateRect = (rect: Rect) => {
  if (rect.animate) {
    ...
    rect.y = Math.random() * 30
  }

  peerService.updateRect(rect)

  const id = window.requestAnimationFrame(() => {
    animateRect(rect);
  });

  if (!animationMaps.has(rect.id)) {
    animationMaps.set(rect.id, id)
  }
}
...
<template>
  <main class="h-full w-full bg-blue-950 flex flex-wrap gap-8 justify-center items-start p-6">
    ...
    <div class="bg-white shadow-md rounded-lg p-8 text-blue-950 max-w-md font-mono">
      ...
      <input type="text" ref="peerId" placeholder="Peer id for now"
        class="w-full border border-blue-950 rounded-lg p-2 mt-4" @keydown.enter="connectPeer" />
      ...
      <div v-if="peerService.peerConnections.value.length > 0">
        ...
        <button class="bg-blue-950 shadow-md rounded-lg p-2 text-white font-mono" @click="addRect">Add Rect</button>

        <div class="mt-4 relative h-56 w-full" v-if="rectStore.rects.length > 0">
          <template v-for="(rect, idx) in rectStore.rects" :key="idx">
            <div class="absolute bg-slate-300 text-slate-800 shadow-sm p-6" @click="() => toggleAnimateRect(rect)"
              :style="{ top: rect.y + 'px', left: rect.y + 'px', background: rect.animate ? 'red' : '' }">{{
                rect.id }}</div>
          </template>
        </div>
        ...
        <!-- <div class="px-8">
                                                ...
                                                <input type="text" ref="message" placeholder="Message"
                                                  class="w-full border border-blue-950 rounded-lg p-2 mt-4 mb-6" @keydown.enter="sendMessage" />
                                              </div>
                                              <div class="w-full rounded-b-lg bg-gray-200 border-t border-gray-400 px-8 pb-6 text-gray-900"
