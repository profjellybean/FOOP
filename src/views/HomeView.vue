...
<template>
  <main class="h-full w-full bg-blue-950 flex flex-wrap gap-8 justify-center items-center">
    ...
    <div class="bg-white shadow-md rounded-lg p-8 text-blue-950 max-w-md font-mono">
      ...
      <input type="text" ref="peerId" placeholder="Peer id for now" class="w-full border border-blue-950 rounded-lg p-2 mt-4" @keydown.enter="connectPeer">
      ...
      <div v-if="peerService.peerConnections.value.length > 0" >
        ...
        <div class="px-8">
          ...
          <input type="text" ref="message" placeholder="Message" class="w-full border border-blue-950 rounded-lg p-2 mt-4 mb-6" @keydown.enter="sendMessage">
        </div>
        <div class="w-full rounded-b-lg bg-gray-200 border-t border-gray-400 px-8 pb-6 text-gray-900" v-if="messageStore.messages.length > 0">
